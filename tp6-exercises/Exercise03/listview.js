...
function display(){
        ...
        for(let i=0 ; i<parseddata.length;i++){
            ...
            `<div class="box_list"> 
                ...
                <div>
                    ...
                    <p class="text2">${typeof parseddata[i].description === "string" && parseddata[i].description.substring(0, 30).concat('...')}</p>
                </div>
            </div>`
        }
    ...
    if(size<81){
        fetch(`https://api.punkapi.com/v2/beers?page=${page}&per_page=${size}`).then(res => res.json()).then((data) => {
            let data_list = data;
